...
public class WhenApplyingProfanityRule
{
    ...
    public async Task Then_When_A_Tested_Field_Contains_A_Profanity_It_Fails(Action<VacancySnapshot, string> setter)
    {
        // arrange
        ...
        var sut = new VacancyProfanityRule(repository.Object);
        
        repository
            .Setup(x => x.GetByContentTypeAsync(ProhibitedContentType.Profanity, It.IsAny<CancellationToken>()))
        ...
        setter(snapshot, "Some content with a rudeword(TM) in");
    ...
    public async Task Then_A_Snapshot_With_No_Offending_Fields_Passes(
        ...
        [Greedy] VacancyProfanityRule sut)
    {
        // arrange
        repository
            .Setup(x => x.GetByContentTypeAsync(ProhibitedContentType.Profanity, It.IsAny<CancellationToken>()))
