...
[TestFixture]
internal class WhenGettingDashboardVacanciesCountByUkprn
{
    [Test, MoqAutoData]
    public async Task GetAllByUkprn_ReturnsPaginatedVacancyDetails(
        ...
        [Greedy] ApplicationReviewsProvider provider)
    {
        ...
        repository.Setup(r => r.GetAllByUkprn(ukprn, pageNumber, pageSize, nameof(ApplicationReviewEntity.CreatedDate), false, status, It.IsAny<CancellationToken>()))
            .ReturnsAsync(paginated);

        // Act
        var result = await provider.GetAllByUkprn(ukprn, pageNumber, pageSize, nameof(ApplicationReviewEntity.CreatedDate), false, status);
        ...
        [Greedy] ApplicationReviewsProvider provider)
    {
        ...
        var paginated = new PaginatedList<ApplicationReviewEntity>([], 0, pageNumber, pageSize);
        repository.Setup(r => r.GetAllByUkprn(ukprn, pageNumber, pageSize, sortColumn, isAscending, status, It.IsAny<CancellationToken>()))
            .ReturnsAsync(paginated)
            .Verifiable();
